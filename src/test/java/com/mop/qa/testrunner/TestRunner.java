		...
		try {
			...
			for (int j = 0; j < testLists.size(); j++) {
				...
				for (int i = 0; i < suiteTest.size(); i++) {
					...
					/*3/5/2019*/
					/*Start*/
					test.addParameter("UserName", suiteTest.get(i).getUserName());
					userName=suiteTest.get(i).getUserName();
					test.addParameter("Password", suiteTest.get(i).getPassword());
					password=suiteTest.get(i).getPassword();
					test.addParameter("URL", suiteTest.get(i).getURL());
					URL=suiteTest.get(i).getURL();
					test.addParameter("Environment", suiteTest.get(i).getEnvironment());
					environment=suiteTest.get(i).getEnvironment();
					/*End*/
				...
				for (int count = 1; count <= sheet.getLastRowNum(); count++) {
					...
					if (row.getCell(2).toString().equalsIgnoreCase("Yes")) {
						...
						for (int l = 0; l < count1 ; l++) {//count as no of instances
							...
							/*3/5/2019*/
							test.setUserName(row.getCell(13).toString());
							test.setPassword(row.getCell(14).toString());
							test.setURL(row.getCell(15).toString());
							test.setEnvironment(row.getCell(16).toString());
